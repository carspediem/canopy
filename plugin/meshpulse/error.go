...
func ErrInvalidMeasurement() *PluginError {
	return NewError(20, "meshpulse", "measurement must have at least one non-zero metric (ping, download, or upload)")
}

func ErrInsufficientRewardBalance() *PluginError {
	return NewError(21, "meshpulse", "contributor does not have enough $MESHP balance to claim that amount")
}
